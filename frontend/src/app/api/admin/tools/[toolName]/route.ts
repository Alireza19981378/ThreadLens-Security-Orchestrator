...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ toolName: string }> }) {
  const { toolName } = await params;
  const response = await fetch(`${backendUrl}/api/v1/admin/tools/${encodeURIComponent(toolName)}/`, {
    method: "PATCH",
